create procedure "informix".create_aging()
...
define err_num            integer;
  ...
  lock table asaaging   in exclusive mode;
  lock table asardetail in exclusive mode;
  -- dave missed this
  lock table strcustr in exclusive mode;
  ...
  if day(NewAgingDate) <= 20 then
    let NewAgingDate = extend(extend(today, year to month), year to day);
    let NewAgingDate = NewAgingDate - 1 units day;
  end if
  ...
  into temp tempDbcr;
  -- create index
  create index tempDbcr_ndx on tempDbcr(cust_code, dbcr );
  ...
  foreach agingrec with hold for
    ...
    if ((PrevBalance <> 0 ) or
      ...
      -- add to test aging table
      insert into asaaging( cust_code, bucket_120, bucket_90, bucket_60,
                            bucket_30, current_bal, prev_stmt_bal,
                            prev_debit, prev_credit, aging_date)
                   values ( CustCode, Bucket120, Bucket90, Bucket60,
                            Bucket30, CurrentBal, PrevBalance,
                            prevDebit, prevCredit, NewAgingDate );

       update strcustr set
         last_stmt_date = NewAgingDate,
         last_stmt_amt  = Bucket120+Bucket90+Bucket60+Bucket30+CurrentBal,
         max_order_amt  = Bucket120+Bucket90+Bucket60+Bucket30
       where cust_code = CustCode;
    ...
    -- update asardetail to set aging date
    update asardetail
        set aging_date = NewAgingDate
    where aging_date is null;

    -- update asaacctsys
    update asaacctsys
      set last_aging_date   = NewAgingDate,
          last_mthly_locker = last_mthly_locker +1 units month;
...
-- Permissions for routine "create_aging"
grant execute on procedure 'informix'.create_aging() to 'public';
