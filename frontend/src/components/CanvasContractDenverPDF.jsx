...
export const generatePDFBuffer = async (formData, signatureData, signatureDate, initialedSections, selectedClub, membershipPrice) => {
  ...
  try {
    ...
    // Add title: for Colorado, draw CAC logo image instead of text; NM keeps text
    const isColorado = !isNewMexicoClub(selectedClub);
    let membershipTextY = 30;
    if (isColorado) {
      try {
        const imgInfo = await loadImageAsDataUrl(CACLogo40);
        ...
        const leftAdjustMm = 6; // small nudge to visually align with title text
    ...
    currentYPos += 5;
    pdf.setFont('helvetica', 'normal');
    const liabilityPropertyText = `I hereby acknowledge and recognize that I am responsible to protect against theft or damage to any of my or my Guests' personal property while using the CAC's facilities and that CAC has advised me not to bring any valuables to CAC' facilities. I waive and release CAC from any and all claims, damages, or responsibility relating to the theft of or damage to my or my Guests' personal property at CAC' facilities, including without limitation, any belongings left by me in a locker, the locker room, or other CAC facilities, or theft or damage to any automobiles or personal property in the club's parking lot.`;
    ...
    currentYPos += 5;
    pdf.setFont('helvetica', 'normal');
    const corporateProofText1 = 'Although you were unable to provide corporate proof when beginning your membership, we would like to offer you the opportunity to immediately take advantage of your membership.';
    const splitCorporateProof1 = pdf.splitTextToSize(corporateProofText1, 160);
    currentYPos = drawPagedText(pdf, splitCorporateProof1, 20, currentYPos);
    currentYPos += 5;
    const corporateProofText2 = `If this proof is not received within 14 days, your membership will be converted to the equivalent of one individually priced membership and you will be responsible for the entire billed amount. If the documentation is not received by ${formData?.requestedStartDate ? calculateCancellationDate(formData.requestedStartDate) : ''}, your rate will go to $150.00 per month until the proper documentation is provided. The club will not issue a dues credit for any portion of the additional charges once billed.`;
    const splitCorporateProof2 = pdf.splitTextToSize(corporateProofText2, 160);
    currentYPos = drawPagedText(pdf, splitCorporateProof2, 20, currentYPos);
    if (initialedSections?.corporateProof && signatureData?.initials?.text) {
      if (currentYPos + 5 > pdf.internal.pageSize.getHeight() - 20) {
        pdf.addPage();
        currentYPos = 20;
      }
