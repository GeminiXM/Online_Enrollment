...
const FluidPayPayment = () => {
 ...
 // (formatDateWithoutTimezoneShift not used on this page)
  ...
  // Calculate prorated amount due now (stable)
  const calculateProratedAmount = React.useCallback(() => {
  ...
  const finishEnrollment = async (paymentResult) => {
    try {
      ...
      const expirationDate = paymentResult.expirationDate || "1225";
      const submissionData = {
        ...
        paymentInfo: {
          ...
          vaultToken: paymentResult.vaultToken || ""
        }
      };
  ...
  // (processPaymentWithToken defined below)
  
  // Function to initialize FluidPay Tokenizer with provided processor info (stable)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const initializeTokenizerWithKey = React.useCallback((processorInfoToUse) => {
    ...
    /* eslint-disable-next-line react-hooks/exhaustive-deps */
  }, [calculateProratedAmount, customerInfo, isTokenizerLoaded]);

  // Initialize Tokenizer when script is loaded and processor info is available
  useEffect(() => {
    if (!(isTokenizerLoaded && processorInfo && window.Tokenizer && !isInitialized)) return;
    const realApiKey = 'pub_31FUYRENhNiAvspejegbLoPD2he';
    const modifiedProcessorInfo = { ...processorInfo, fluidpay_api_key: realApiKey };
    const tokenizerInstance = initializeTokenizerWithKey(modifiedProcessorInfo);
    if (tokenizerInstance) {
      setTokenizer(tokenizerInstance);
      setIsInitialized(true);
    }
  }, [isTokenizerLoaded, processorInfo, isInitialized, initializeTokenizerWithKey]);
  
  // Match Converge logic: 1st of next month (displayed as mm/dd/yyyy)
...
const launchPayment = async () => {
  ...
  // (finishEnrollment defined above)
  ...
  // (calculateProratedAmount defined above as a stable callback)
      ...
      <div className="payment-layout">
        ...
        <div className="payment-form-container">
          ...
          <div className="lightbox-explainer">
            ...
            <p>
              Enter your payment information securely below. Your card details are tokenized and never stored on our servers.
            </p>
