...
const PretendPaymentPage = () => {
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      const mockPaymentResponse = {
        processor: 'PRETEND',
        success: true,
        transaction_id: 'PRETEND_' + Date.now(),
        authorization_code: 'AUTH' + Math.random().toString(36).substr(2, 6).toUpperCase(),
        card_info: {
          last_four: paymentFormData.cardNumber.replace(/\s/g, '').slice(-4),
          card_type: getCardType() || 'Credit Card'
        },
      ...
      setTimeout(() => {
        finishEnrollment(mockPaymentResponse);
      }, 2000);
  ...
  // Complete enrollment after pretend payment
  const finishEnrollment = async (paymentResult) => {
    try {
      ...
      // Email will be sent from EnrollmentConfirmation page
      console.log('Email will be sent from confirmation page');
      ...
      navigate('/enrollment-confirmation', { 
        state: { 
          ...
          amountBilled: calculateProratedAmount().toFixed(2) // Use the correct calculated amount
        } 
      });
    } catch (error) {
  ...
  const getFirstOfNextMonth = () => {
    ...
    // Process pretend Converge payment
const processConvergePayment = async () => {
  ...
  try {
    ...
    // Create mock payment response
    const mockPaymentResponse = {
      processor: 'PRETEND_CONVERGE',
      success: true,
      transaction_id: 'PRETEND_CONVERGE_' + Date.now(),
      authorization_code: 'AUTH' + Math.random().toString(36).substr(2, 6).toUpperCase(),
      card_info: {
...
export default PretendPaymentPage;
