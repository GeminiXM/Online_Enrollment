...
export const getCCProcessorName = async (req, res) => {
  try {
    ...
    try {
      ...
      const result = await executeSqlProcedure("web_proc_GetCCProcessorName", clubId, [clubId]);
...
export const getFluidPayInfo = async (req, res) => {
  try {
    ...
    try {
      ...
      const result = await executeSqlProcedure("procFluidPayItemSelect1", clubId, [clubId]);
      ...
      if (!fluidPayInfo) {
        ...
        logger.warn("No FluidPay info found for club, using fallback", { clubId });
        fluidPayInfo = {
...
export const getConvergeInfo = async (req, res) => {
  try {
    ...
    try {
      // Execute the stored procedure
      const result = await executeSqlProcedure("procConvergeItemSelect1", clubId, [clubId]);
      ...
      if (result && result.length > 0) {
        ...
        if (firstRow) {
          convergeInfo = {
            ...
            converge_url_process_batch: firstRow.converge_url_process_batch || "",
      ...
      if (!convergeInfo) {
        // Provide fallback demo data
        logger.warn("No Converge info found for club, using fallback", { clubId });
        convergeInfo = {
          merchant_id: `cvmerchant_${clubId}`,
          converge_user_id: "webuser",
          converge_pin: "12345",
          converge_url_process: "https://api.demo.convergepay.com/VirtualMerchantDemo/processTransaction.do",
          converge_url_process_batch: "https://api.demo.convergepay.com/VirtualMerchantDemo/processBatch.do",
...
export const processPaymentDemo = async (req, res) => {
  try {
    ...
    // Input validation
    if (!clubId || !cardNumber || !expiryDate || !cvv || !nameOnCard || !billingZipCode) {
    ...
    logger.info(`Processing demo payment for club ID: ${clubId} using ${processorName || 'unknown'} processor`);
    ...
    if (!processorToUse) {
      ...
      const processorResult = await executeSqlProcedure("web_proc_GetCCProcessorName", clubId, [clubId]);
    ...
    if (processorToUse === 'FLUIDPAY') {
      // Get FluidPay info
      const fluidPayResult = await executeSqlProcedure("procFluidPayItemSelect1", clubId, [clubId]);
      let merchantId = "";
      ...
      if (isDeclineCard) {
        ...
        paymentResponse = {
          success: true,
          processor: 'FLUIDPAY',
          transaction_id: `fp_${Date.now()}_${Math.floor(Math.random() * 1000000)}`,
          merchant_id: merchantId,
          authorization_code: Math.floor(Math.random() * 1000000).toString().padStart(6, '0'),
          card_info: {
            last_four: cardNumber.slice(-4),
            card_type: cardNumber.startsWith('4') ? 'visa' : 
      ...
      // Assume CONVERGE
      // Get Converge info
      const convergeResult = await executeSqlProcedure("procConvergeItemSelect1", clubId, [clubId]);
      let merchantId = "";
      ...
      if (isDeclineCard) {
        paymentResponse = {
          ...
          error: {
            ssl_result: "1",
            ssl_result_message: "Your card was declined. Please try a different payment method."
          },
          ssl_txn_id: null
        };
      } else {
        paymentResponse = {
          ...
          ssl_approval_code: Math.floor(Math.random() * 1000000).toString().padStart(6, '0'),
          ssl_card_number: `XXXXXXXXXXXX${cardNumber.slice(-4)}`,
          ssl_card_type: cardNumber.startsWith('4') ? 'VISA' : 
                        cardNumber.startsWith('5') ? 'MASTERCARD' : 
