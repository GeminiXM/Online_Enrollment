...
import {
  ...
  getConvergeInfo,
  getConvergeToken,
  processConvergeRebill,
  processConvergeDirectPayment,
  testConvergeCredentials,
  processFluidPayPayment,
  processPaymentDemo,
  testConvergeApiAccess,
 ...
 * @desc Get Converge session token for hosted payment fields
 * @access Public
 */
router.post("/converge-token", async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message:
        "An error occurred while generating the Converge session token. Please try again later.",
    });
  }
});
 ...
 * @access Public
 */
router.post("/converge-rebill", async (req, res) => {
  try {
    return await processConvergeRebill(req, res);
  } catch (error) {
    logger.error("Error in converge-rebill route", {
      error: error.message,
      stack: error.stack,
    });
    return res.status(500).json({
      success: false,
      message:
        "An error occurred while processing the Converge rebill payment. Please try again later.",
    });
  }
});
 ...
 * @desc Test Converge credentials by making API calls
 * @access Public
 */
router.post("/converge-test-credentials", async (req, res) => {
  try {
    return await testConvergeCredentials(req, res);
  } catch (error) {
    logger.error("Error in converge-test-credentials route", {
      error: error.message,
      stack: error.stack,
    });
    return res.status(500).json({
      success: false,
      message:
        "An error occurred while testing Converge credentials. Please try again later.",
    });
  }
});
 ...
 * @access Public
 */
router.post("/converge-api-test", async (req, res) => {
  try {
    return await testConvergeApiAccess(req, res);
  } catch (error) {
    logger.error("Error in converge-api-test route", {
      error: error.message,
      stack: error.stack,
    });
    return res.status(500).json({
      success: false,
      message:
        "An error occurred while testing Converge API access. Please try again later.",
    });
  }
});
 ...
 * @desc Process Converge payment directly through VirtualMerchant
 * @access Public
 */
router.post("/converge-direct-payment", async (req, res) => {
  try {
    return await processConvergeDirectPayment(req, res);
  } catch (error) {
    logger.error("Error in converge-direct-payment route", {
      error: error.message,
      stack: error.stack,
    });
    return res.status(500).json({
      success: false,
      message:
        "An error occurred while processing the direct Converge payment. Please try again later.",
    });
  }
});
